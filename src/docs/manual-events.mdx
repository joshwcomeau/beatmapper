...
> This view can look a little weird/overwhelming, especially if you're coming from another editor. Once you get the hang of it, though, this interface will let you create beautiful, advanced lighting patterns really quickly and easily. Give it a chance!
...
## Shared controls with the Notes view

Whenever possible, I've tried to mirror controls between the two primary views. This includes:

- Time navigation controls, like playing/pausing, scrubbing, seeking, etc.
- `Tab` rotates between tools
- Colors are selected with `R` and `B` (except now we're selecting light colors, instead of block colors)
- Actions like Quick-select (`Q`) and Jump (`J`) work the same way.
